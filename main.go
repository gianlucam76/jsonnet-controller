...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "7a7add09.projectsveltos.io",
	})
	if err != nil {
	...
	var jsonnetController controller.Controller
	jsonnetReconciler := (&controllers.JsonnetSourceReconciler{
		Client:               mgr.GetClient(),
		Scheme:               mgr.GetScheme(),
		ReferenceMap:         make(map[corev1.ObjectReference]*libsveltosset.Set),
		JsonnetSourceMap:     make(map[types.NamespacedName]*libsveltosset.Set),
		PolicyMux:            sync.Mutex{},
		ConcurrentReconciles: concurrentReconciles,
	})
	jsonnetController, err = jsonnetReconciler.SetupWithManager(mgr)
	if err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "JsonnetSource")
		os.Exit(1)
	}
	...
	go fluxWatchers(ctx, mgr,
		jsonnetReconciler, jsonnetController,
		setupLog)
...
func fluxWatchers(ctx context.Context, mgr ctrl.Manager,
	jsonnetSourceReconciler *controllers.JsonnetSourceReconciler, jsonnetSourceController controller.Controller,
	logger logr.Logger) {
	...
	for {
		...
		if err != nil {
			...
			if !fluxPresent {
				...
				err = jsonnetSourceReconciler.WatchForFlux(mgr, jsonnetSourceController)
				if err != nil {
					continue
				}
			}
