...
create-cluster: $(KIND) $(KUBECTL) $(ENVSUBST) ## Create a new kind cluster designed for development
	...
	@echo "Start jsonnet-controller"
	$(MAKE) deploy-jsonnet-controller

	@echo "prepare configMap with jsonnet file using variables"
	$(KUBECTL) create configmap jsonnet-with-variables --from-file=jsonnet.tar.gz=test/jsonnet.tar.gz

	@echo "prepare configMap with jsonnet file importing other files"
	$(KUBECTL) create configmap jsonnet-with-import --from-file=jsonnet.tar.gz=test/jsonnet_multiple_files.tar.gz

.PHONY: delete-cluster
delete-cluster: $(KIND) ## Deletes the kind cluster $(CONTROL_CLUSTER_NAME)
	$(KIND) delete cluster --name $(CONTROL_CLUSTER_NAME)

deploy-jsonnet-controller: $(KUSTOMIZE)
	# Load jsonnet controller image into cluster
	@echo 'Load jsonnet-controller image into cluster'
	$(MAKE) load-image
	...
	$(KUBECTL) wait --for=condition=Available deployment/jsonnet-controller -n jsonnet-system  --timeout=$(TIMEOUT)
