...
func ConfigMapPredicates(logger logr.Logger) predicate.Funcs {
	return predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if oldConfigMap == nil {
				log.V(logs.LogVerbose).Info("Old ConfigMap is nil. Reconcile JsonnetSources.")
				return true
			}

			if !reflect.DeepEqual(oldConfigMap.BinaryData, newConfigMap.BinaryData) {
				log.V(logs.LogVerbose).Info(
					"ConfigMap Data changed. Will attempt to reconcile associated JsonnetSources.",
				)
				return true
			}
			...
			log.V(logs.LogVerbose).Info(
				"ConfigMap did not match expected conditions.  Will not attempt to reconcile associated JsonnetSources.")
			return false
		},
...
func SecretPredicates(logger logr.Logger) predicate.Funcs {
	return predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if oldSecret == nil {
				log.V(logs.LogVerbose).Info("Old Secret is nil. Reconcile JsonnetSources.")
				return true
			}

			if !reflect.DeepEqual(oldSecret.Data, newSecret.Data) {
				log.V(logs.LogVerbose).Info(
					"Secret Data changed. Will attempt to reconcile associated JsonnetSources.",
				)
				return true
			}
			...
			// otherwise, return false
			log.V(logs.LogVerbose).Info(
				"Secret did not match expected conditions.  Will not attempt to reconcile associated JsonnetSources.")
			return false
		},
		CreateFunc: func(e event.CreateEvent) bool {
			return CreateFuncTrue(e, logger)
...
// JsonnetSourceReconciler watches GitRepository/OCIRepository/Bucket events and
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if hasArtifactChanged(s, e) {
				log.V(logs.LogInfo).Info(
					"Source artifact has changed.  Will attempt to reconcile associated JsonnetSources.")
				return true
			}

			// otherwise, return false
			log.V(logs.LogInfo).Info(
				"GitRepository did not match expected conditions.  Will not attempt to reconcile associated JsonnetSources.")
			return false
		},
		CreateFunc: func(e event.CreateEvent) bool {
			log := logger.WithValues("predicate", "createEvent",
			...
			log.V(logs.LogVerbose).Info(
				"Source did match expected conditions.  Will attempt to reconcile associated JsonnetSources.")
			return true
		},
		DeleteFunc: func(e event.DeleteEvent) bool {
			log := logger.WithValues("predicate", "deleteEvent",
			...
			log.V(logs.LogVerbose).Info(
				"Source deleted.  Will attempt to reconcile associated JsonnetSources.")
			return true
		},
		GenericFunc: func(e event.GenericEvent) bool {
			log := logger.WithValues("predicate", "genericEvent",
			...
			log.V(logs.LogVerbose).Info(
				"Source did not match expected conditions.  Will not attempt to reconcile associated JsonnetSources.")
			return false
		},
	}
}
