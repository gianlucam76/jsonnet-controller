...
var _ = Describe("JsonnetSourceTransformation map functions", func() {
	It("RequeueJsonnetSourceForReference returns JsonnetSource referencing a given ConfigMap", func() {
		...
		reconciler := &controllers.JsonnetSourceReconciler{
			Client:           c,
			Scheme:           scheme,
		...
		Expect(requests).To(ContainElement(
			reconcile.Request{NamespacedName: types.NamespacedName{Namespace: JsonnetSource1.Namespace, Name: JsonnetSource1.Name}}))
	})
})

var _ = Describe("JsonnetSourceTransformation map functions", func() {
	It("RequeueJsonnetSourceForFluxSources returns JsonnetSource referencing a given GitRepository", func() {
